...
export class Worker {
	...
	private async connectToServer(): Promise<any> {
		const client: any = new ImapClient.default(Worker.serverInfo.imap.host, Worker.serverInfo.imap.port, { auth: Worker.serverInfo.imap.auth, requireTLS:true });
    ...
    public async listMessages(inCallOptions: ICallOptions): Promise<IMessage[]> {
        ...
        const messages: any[] = await client.listMessages(
            inCallOptions.mailbox, "1:50" , ["uid", "envelope"]
        );
        await client.close();
    ...
    public async getMessageBody(inCallOptions: ICallOptions): Promise<string> {
        ...
        await client.close();
        return parsed.text || "nothing to see";
