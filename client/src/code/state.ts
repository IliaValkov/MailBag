...
export function createState(inParentComponent: React.Component) {
	return {
		...
		updateContact: async function(): Promise<void> {
			if(this.state.currentView === "contact") {
				this.setState({currentView: "contactUpdate"})
			}else{
				this.state.showHidePleaseWait(true);
				const contactsWorker: Contacts.Worker = new Contacts.Worker();
				const contactUpdateInfo: Contacts.IContact = {
					_id: this.state.contactID, 
					name: this.state.contactName, 
					email: this.state.contactEmail};
				await contactsWorker.updateContact(contactUpdateInfo);
				this.state.showHidePleaseWait(false);
				const cl = this.state.contacts.slice(0);
				cl.forEach((contact: Contacts.IContact ) => {
					if(contact._id === this.state.contactID){
						Object.assign(contact, contactUpdateInfo);
					}
				});
				this.setState({
					contacts: cl,
